...
let read file = 
    ...
    |> Array.map (fun line -> 
        // TODO split the line, create record
        )
...
data
|> Array.map // TODO extract X and Y coordinates
...
|> Array.map (fun (label, group) ->
    // TODO for each group, extract coordinates
    )
|> Chart.Scatter
|> Chart.WithSize (800, 640)
|> Chart.Show
...
let distance flower1 flower2 = 
    // TODO compute the sum of 
    // the square of petal length differences, and
    // the square of petal width differences
...
let clusterize (n: int) (data: Flower []) =
    ...
    let rec update (currentClusters: (Flower * Flower []) []) =
        printfn "Trying to refine clusters"
        let updatedCentroids = 
            // compute the centroids for the current clusters
            currentClusters 
            |> Array.map (fun (_, cluster) -> // TODO
        // if the centroids are the same as before, we are done
        if 
            updatedCentroids 
        ...
        // otherwise we repeat, clustering using the updated centroids
        else 
            // TODO  

    // we initialize by creating 3 random centroids first,
    // using 3 random observations from the dataset.
    let rng = Random 0
...
|> Array.groupBy snd
|> Array.map (fun (centroid, group) -> // TODO
